...
import type { Athlete, DB, Event, NationalsConfig, Registration, Score } from '../../src/lib/types';
import {
  platformCategory,
  buildEntries,
...
function scenario() {
  ...
  return {
    db, event, registrations: registrations as unknown as Registration[], scores: scores as unknown as Score[],
    people: people as unknown as Athlete[],
  };
}
...
describe('nationals adapter', () => {
  ...
  it('builds engine entries from registrations + scores', () => {
    const { db, event, registrations, people, scores } = scenario();
    const entries = buildEntries(db, event, registrations, people, 'WAG', scores, 'prelim');
  ...
  it('computes prelim AA placement + qualification through the engine', () => {
    const { db, event, registrations, people, scores } = scenario();
    const { prelims } = computeArtisticDiscipline(db, event, registrations, people, 'WAG', scores);
  ...
  it('computeNationals returns a WAG bundle for a Nationals event', () => {
    const { db, event, registrations, people, scores } = scenario();
    const bundle = computeNationals(db, event, registrations, people, scores);
