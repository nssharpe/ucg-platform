...
import type { AthleteEntry, AthleteResult, DisciplineDef, ApparatusPlacement, QualFlag } from './types';
...
/**
 * Compute TNT placement + qualification. Each apparatus has its own level; grouping
 * and blue numbers are per (apparatus-level), with no category, AA, team, or 50%
...
export function computeTnt(
  ...
  const results: AthleteResult[] = entries.map((e) => {
    ...
    return { entry: e, category: e.category, level: '', apparatus };
  });

  for (const ev of def.apparatus) {
    const rows = results.map((r) => ({
      id: r.entry.id,
      score: r.apparatus[ev].score,
      level: byId.get(r.entry.id)!.apparatus[ev]?.level ?? '',
    }));
    const places = assignPlaces(
      rows,
      (row) => byId.get(row.id)!.apparatus[ev]?.status !== 'Scratched',
      (row) => row.level,
    ...
    for (const r of results) {
      ...
      const level = r.entry.apparatus[ev]?.level ?? '';
