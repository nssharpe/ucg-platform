...
import { newRegistrationEntryTotal, registrationChangeFee, changeIsEligible, syncSynchroPartnerLevel, lateFeeApplies } from '../lib/pricing';
...
function MyRegistrationsInner({ personId }: { personId: string }) {
  ...
  const saveRegs = (event: Event, selectedClubId: string, newRegs: Registration[]) => {
    ...
    mutate((d) => {
      ...
      // first (rule 2: once per athlete+event, not per discipline). This branch
      // only runs when `!editingExisting` (existingForAthlete is empty), so the
      // anchor is always "now" here — computed generally for defensiveness.
      const lateAnchor = (() => {
