...
> (retryable partial failure) + an amount-reconciliation check. Phase 3: M1 (coupon
> reservation) shipped 2026-07-26; M2/M4 + the invoice write lockdown shipped
> 2026-07-24. The three **LOW** findings (`club_managers`/`app_settings` anon read,
> `error_logs` insert spam, token entropy) are drafted 2026-07-26 on branch
> `sec/p3-low-items`, not yet applied/deployed — see
> `docs/plans/2026-07-02-security-hardening.md` Phase 3 status note for detail.
> **Part 2 (client cart state machine — C5, H5–H8, M6–M9, L2)
