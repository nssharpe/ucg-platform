...
| A-07-01 (Nate) | S2→S3 | "+ New Person" doesn't invite | Partly plan wording (fixed in the step). Add a "Send account invite now" checkbox to the New Person dialog so the obvious path works too | ✅ 2026-08-22 — checkbox wired to the existing `createAccountInvite` (signup-link) flow, not the branded `invite-account` edge function. **Superseded 2026-08-25 (UAT round 2 A-07-01):** `createAccountInvite`/`resendAccountInvite`/the New-Person checkbox now ALL route through `invite-account` (branded set-password link), the accountInvites row is bookkeeping only — see the round-2 section in the notes file |
