...
| `20260718200055_season_launched_at.sql` | **F6 (season lifecycle): `seasons.launched_at timestamptz`.** null = not launched; launched ⇒ memberships purchasable (enforced server-side in `create-checkout-session` via `seasonPurchasableForCheckout` — current OR launched; past-but-active late-signup behavior preserved) and events creatable in that season (`eventCreationBlocked`, EventWizard). Backfill: `launched_at = now()` for any season that is `current` or has memberships sold. Drives `scheduled-dispatch`'s season consumer: escalating admin-only nags (`season-launch-nag`; june1/june16 once per fiscal cycle, daily once per UTC day from June 24 and past July 1 until launched) + the automatic `current` flip (`season-rollover`; set-new-then-clear-old so a crash leaves two currents that self-heal next run, never zero; one-time admin email keyed by target season id). Pure logic in `src/lib/season-lifecycle.ts`, mirrored in `_shared/season-lifecycle.ts` — keep in lockstep. **Applied staging + prod 2026-07-18** (backfill verified on both). |
