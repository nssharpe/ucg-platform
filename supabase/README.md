...
| `20260718142708_recon_note_column.sql` | **F4 (payments reconciliation): `payments.recon_note text`.** Bookkeeping-only note column written exclusively by the new `reconcile-payments` edge function's `mark-refunded` op, to record a CONFIRMED Stripe-Dashboard-side refund found via the `scan` op — no new status value or audit table. A FULL drift (Stripe refunded the entire charge) also flips `payments.status` to the existing `'refunded'` value; a PARTIAL drift leaves `status` alone (no partial-refunded value exists) and relies on this note. No client write policy (mirrors `refund_requests`'s server-only write model). **NOT YET APPLIED — drafted on `feat/payments-recon`, staging first then prod per the standing migration order.** |
...
`reconcile-payments` `scan`/`mark-refunded` ops (Finance page "Reconciliation" tab, NOT YET
DEPLOYED) close this loop as a manual admin action**, not an automatic one.
