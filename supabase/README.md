...
| `20260718093940_is_admin_passkey_amr.sql` | **Passkey exemption for the aal2-conditional `is_admin()`** (Nate, 2026-07-18): a passkey SIGN-IN session stays aal1 in GoTrue but its server-signed `amr` claim carries method `'passkey'` (verified against supabase/auth source) — `is_admin()` now also accepts that, since a passkey is already possession + user-verification and demanding TOTP on top is pure burden. Fail-closed (`coalesce(bool_or(...), false)`, `jsonb_typeof` guard). **Lockstep rule:** the client gate (`needsMfaStepUp`, `src/lib/mfa-core.ts`) and the edge guard (`_shared/jwt-aal.ts` `callerAalSatisfies`) carry the same exemption — change all three together or passkey-signed-in TOTP-enrolled admins get locked out. **Applied staging + prod 2026-07-18** (9 guarded functions redeployed prod, 8 staging — admin-reset-mfa stays prod-only). |
| `20260717205043_error_logs_retention.sql` | **`error_logs` 90-day retention** (privacy note in docs/research/2026-06-22-error-logging-observability.md): pg_cron job `error-logs-retention-daily` (04:10 UTC) deletes rows older than 90 days. **Applied staging + prod 2026-07-17.** |
| `20260717205348_camp_survey_scoped_read.sql` | **Privacy fix: `registrations.camp_survey` is no longer world-readable (docs/research/2026-07-17-supabomb-scan-results.md).** `registrations` carries `public_read using (true)` from the original RLS migration (intentional — drives live results/start lists keyed on opaque ids), but `camp_survey` (added later, `20260618200000`, holds camp overnight-accommodation answers incl. cabin gender preference + a free-text roommate request, for registrants who are sometimes minors) rode along on that same public policy without ever being re-scoped. Fix has three parts: (1) new SECURITY DEFINER RPC **`registration_camp_surveys(p_event_id text default null)`** returning `(registration_id text, camp_survey jsonb)` — authorization is per-ROW in the WHERE clause (the athlete themself via `my_person_id()`, a manager of the registration's own club via `manages_club(club_id)`, or the event's host via the existing `is_event_host(event_id)` helper from `20260710020303_host_post_close_edit.sql`); `revoke execute ... from public` + `grant execute ... to authenticated`. (2) **`revoke select on registrations from anon, authenticated` + `grant select (`every column except `camp_survey`) on registrations to anon, authenticated`** — a bare column-level `revoke select (camp_survey) ...` would be a no-op here: anon/authenticated never got an explicit table grant from any migration (same as the finance-foundations migration's own documented finding for payments/invoices/etc.) — their SELECT access has always come from the project's default Supabase bootstrap privileges (a blanket `grant all on all tables`), and Postgres ACL checks union every applicable grant, so that pre-existing table-wide grant would still cover camp_survey regardless of a narrower column-level revoke. Revoking the table grant and re-granting the explicit column list is the same idiom already used for `waitlist_groups`/`event_checkins`' `revoke all` + `grant update (status, ...)`; `public_read using (true)` still decides which ROWS are visible, this only narrows which COLUMNS of a visible row are readable. SECURITY DEFINER functions run as the function owner, so none of this affects `registration_camp_surveys()` or the pre-existing `event_host_roster()` (which already returns `camp_survey` to hosts, unaffected). (3) App-side: `src/lib/supabase.ts`'s `loadAll` now selects an explicit `registrations` column list EXCLUDING `camp_survey` (`REGISTRATION_COLUMNS_NO_SURVEY`, a literal string so postgrest-js's select-string type inference narrows the return type — `rowToRegistration`'s parameter type was loosened to make the column optional accordingly) instead of `select('*')` (which would otherwise ask for a column PostgREST can no longer return, erroring the whole query). New invoker `fetchCampSurveys(eventId?)` wraps the RPC. Write-path check (no migration/app change needed): every `registrations` write in `src/lib/supabase.ts` goes through `.upsert()`/`.update()`/`.delete()` with no `.select()` chained, so postgrest-js already sends `Prefer: return=minimal` — the column-level SELECT revoke cannot break writing `camp_survey` since no representation is ever requested back. FE consumers: `Events.tsx`'s `SelfRegModal` and `MyRegistrations.tsx`'s `EditRegistrationModal` (both self-registration flows that prefill a prior answer) now call `fetchCampSurveys(event.id)` in a `useEffect` and merge the result into the survey draft's initial state; the host workbook path (`Events.tsx` host view + `src/lib/host-export.ts`) was already reading `camp_survey` exclusively through `event_host_roster()`, so it needed no change. **Applied staging + prod 2026-07-17**; anon-probe verified on both (plain columns 200, `camp_survey` select 42501, `select=*` 42501). |
| `20260717211754_camp_survey_rpc_revoke_anon.sql` | **Follow-up to the row above:** the post-apply anon probe showed anon could still EXECUTE `registration_camp_surveys` (returning `[]` — the WHERE clause fails closed, no data leaked). Supabase's `ALTER DEFAULT PRIVILEGES` grants EXECUTE on new functions to anon/authenticated/service_role directly, so `revoke ... from public` alone leaves anon's own grant standing (same ACL-union trap as the table grant). This revokes anon's execute explicitly; probe now returns 42501. **Applied staging + prod 2026-07-17.** |
| `20260718200055_season_launched_at.sql` | **F6 (season lifecycle): `seasons.launched_at timestamptz`.** Originally: null = not launched; launched ⇒ memberships purchasable and events creatable in that season, plus an automatic `current` flip driven by `scheduled-dispatch`. **Superseded by P3 (2026-07-20, `docs/specs/2026-07-20-season-card-ucg-events-and-cleanups.md`): the app stops reading/writing `seasons.current`/`launched_at` entirely** — "current" and "purchasable" are now derived from today's date vs. each season's `[startsOn, endsOn]` window (`purchasableSeasons`/`currentSeason` in `src/lib/season-lifecycle.ts`, mirrored in `_shared/season-lifecycle.ts`; server gate in `_shared/stripe.ts` `seasonPurchasableForCheckout`). The automatic `current` rollover is GONE (no flag to flip); `scheduled-dispatch`'s `season-launch-nag` now just nags admins to CREATE the next season row via "Copy → next year". The `current`/`launched_at` columns stay in the DB (no destructive migration) but are dead weight — nothing reads them anymore. |
| `20260718142708_recon_note_column.sql` | **F4 (payments reconciliation): `payments.recon_note text`.** Bookkeeping-only note column written exclusively by the new `reconcile-payments` edge function's `mark-refunded` op, to record a CONFIRMED Stripe-Dashboard-side refund found via the `scan` op — no new status value or audit table. A FULL drift (Stripe refunded the entire charge) also flips `payments.status` to the existing `'refunded'` value; a PARTIAL drift leaves `status` alone (no partial-refunded value exists) and relies on this note. No client write policy (mirrors `refund_requests`'s server-only write model). **Applied staging + prod 2026-07-18.** |
| `20260719120000_judge_access_codes.sql` | **Codeless judge access (PM decision 2026-07-19): `judge_access_codes` table.** One ACTIVE (`revoked_at is null`) row per event — enforced by a partial unique index on `event_id` — holding a long `token`, a 6-digit `code`, and audit fields. RLS mirrors `is_event_host(event_id)` (the same helper `event_host_scores_write` uses on `scores`) for SELECT/INSERT/UPDATE as three SEPARATE policies (not `for all` — the CLAUDE.md DELETE-grant trap); rows are revoked, never hard-deleted. NO anon/authenticated baseline read — the only way an anonymous device ever resolves a code/token is the new `judge-entry` edge function's service-role client. **Applied staging + prod 2026-07-19.** |
| `20260719130000_event_scoring_config.sql` | **Per-event scoring config (PM decision 2026-07-19): `events.scoring_config jsonb`** (`{panels: 1\|2, entryMode: 'calculator'\|'simple'}`, absent ⇒ default via `scoringConfigOf`) **+ `scores.deductions2 numeric`/`scores.e_score2 numeric`** (second judge panel's raw execution inputs, averaged with `deductions`/`e_score` client-side via `combinePanels` (`src/scoring/panels.ts`) into `final`). Plain column adds — no RLS/grant changes (no column-level grants exist on either table; new columns inherit the existing table grants + RLS policies, same precedent as `20260601000004_text_ids_score_extras.sql`'s `scores.calc`/`calc_state`). **Applied staging + prod 2026-07-19.** |
| `20260720142948_delete_stuck_dev_pending_payments.sql` | **One-time cleanup:** delete stuck-pending `payments` rows older than 1 hour (same criteria as `isStuckPending()`, `src/lib/reconciliation.ts`) — abandoned dev/test `cs_test_...` Checkout sessions from 2026-06-26 that never reached fulfillment (no invoice/`registrations.paid` side-effects; `refund_requests.payment_id` FK is `on delete set null`). Authored 2026-07-20 in an earlier session; committed with the P4 batch. **Applied prod 2026-07-20 (earlier session); staging 2026-07-20.** |
| `20260720212144_events_ucg_hosted.sql` | **UCG-hosted event marker (spec `docs/specs/2026-07-20-season-card-ucg-events-and-cleanups.md`): `events.ucg_hosted text`** (nullable; `'flipfest' \| 'nationals'` written by the app, no enum/constraint). Identifies a season's FlipFest/Nationals instance for the Seasons & fees card's Create/Edit buttons (`findUcgEvent`: matching `ucg_hosted` + `start_date` inside the season window). Plain column add — inherits existing `events` grants/RLS. **Applied staging 2026-07-20; prod 2026-07-22 (Nate).** No longer purely client-only as of 2026-07-22: `request-refund` treats `ucg_hosted` as refund-eligible — see `20260722220449`. |
| `20260722220449_guard_events_ucg_hosted.sql` | **Admin-only guard on `events.ucg_hosted`:** BEFORE INSERT/UPDATE trigger `events_guard_ucg_hosted` — non-privileged callers (not service role/`is_admin()`) may not set or CHANGE `ucg_hosted`; unchanged round-trips (the app's whole-row upserts) pass. Needed because the 2026-07-22 feedback pass made `ucg_hosted` grant refund eligibility (`request-refund`) + "hosted by UCG" branding, and events rows are writable by host-club managers. Follows the re-SELECT-pre-write-row upsert-trap pattern (`20260703034325`) and fail-closed `coalesce(..., false)` privilege check (`20260704133502`); default-privileges execute revoked. **Applied staging 2026-07-22; prod PENDING (in-session CLI push classifier-blocked — Nate runs `supabase db push`).** |
| `20260722221027_events_listing_only.sql` | **Two-tier Nationals publish (PM feedback 2026-07-22): `events.listing_only boolean not null default false`.** True = published "Dates and Location Only" from the UCG Nationals wizard — listed on the Events page WITHOUT a Details link and with registration suppressed; full-details publish clears it. Plain column add, inherits events grants/RLS. Client compat: `eventToRow` OMITS the column when `Event.listingOnly` is undefined and only the Nationals two-button flow stamps it, so ordinary event saves keep working against a DB that predates this migration. **Applied staging 2026-07-22; prod PENDING (same batch as above — Nate runs `supabase db push`; the dates-only publish feature no-ops in prod until then).** |
...
| `admin-delete-person` | **F5 (data export/delete; deployed staging + prod 2026-07-18, staging-smoke-tested: hard-delete on an unreferenced person, tombstone-in-place on a person with a paid registration, wrong-confirmName refusal, non-admin 403).** Admin-operated delete/anonymize: DELETES rows with no financial/legal significance (never-paid registrations, cart lines incl. `ref_user_id` club-cart lines, waitlist groups, guardian sign-request links, non-approved refund requests, account invites, event-admin grants, alt-club links); RETAINS+ANONYMIZES financial/competition history (scrubs the person's denormalized full name out of `invoice_items.label` and `payments.lines_snapshot[].label` — every line kind sets `ref_user_id`, which is the scrub filter); KEEPS `waiver_signatures` untouched (legal retention, pending counsel). The `people` row is tombstoned IN PLACE (name → 'Deleted user', PII blanked, `auth_user_id` → null) whenever registrations/invoices/invoice_items/payments/refund_requests/waiver_signatures still reference it (several are ON DELETE CASCADE — hard delete would destroy paid history); hard-deleted only when nothing references it. Deletes the auth user via the admin API (best-effort, after DB work). Guards: `confirmName` must match the CURRENT full name server-side; refuses club managers (remove manager rows first) and self-delete. Returns a per-step manifest; failures log to `error_logs`; fail-safe defaults to tombstone if ref-count queries error. `verify_jwt` stays `true`. | admin only (AAL guard) |
| `reconcile-payments` | **F4 (payments reconciliation; deployed staging + prod 2026-07-18, staging-smoke-tested: scan finds real stuck rows, refulfill refuses an unpaid session, non-admin 403).** Admin/finance_admin closes the loop on two known Stripe↔DB drifts. `scan` (read-only): (1) stuck-pending `payments` (`status='pending'`, older than 1h); (2) refund drift — `paid` payments in the last `days` (default 90, capped 365, ≤100 Stripe lookups/call, `truncated` flag if more) whose Stripe charge `amount_refunded` doesn't match our approved `refund_requests` total (`classifyPaymentDrift`, `_shared/reconciliation.ts`, mirrors `src/lib/reconciliation.ts`). `refulfill`: re-checks a stuck-pending payment's Stripe session server-side FIRST — only fulfills (via the shared idempotent `fulfillPayment` core) if Stripe confirms `payment_status: 'paid'`; a free-order row (`stripe_session_id` null) re-runs the core directly. `mark-refunded`: re-verifies against Stripe before writing `payments.recon_note` (+ `status: 'refunded'` for a FULL drift only) — bookkeeping alignment ONLY, never touches registrations/memberships/invoices/`refund_requests`. `verify_jwt` stays `true`. | admin / finance_admin (AAL guard) |
| `judge-entry` | **Codeless judge access (PM decision 2026-07-19, on `feat/judge-access-codes` — not yet deployed).** Two ops for an anonymous device: `unlock` (resolve a 6-digit `code` or long `token` from `judge_access_codes` to `{eventId, token}`; a code matching >1 active row across events is rejected rather than guessed — "use the link/QR instead"; failed attempts logged to `error_logs` + a ~300ms soft brake) and `submit` (validate the token is active + the event is `status:'live'`, then validate the score via `_shared/judge-entry-core.ts` — apparatus-in-registration, refunded check, numeric bounds — and upsert into `scores` with `entered_by = 'judge-code:'+codeRowId` stamped server-side; the score id is ALWAYS recomputed from the token's event + payload, never trusted from the client). `verify_jwt` stays `true` (same anon-key-passes-the-gateway shape as `record-waiver-signature`) — NOT added to the `--no-verify-jwt` trio. No AAL guard (anonymous by design); every write is confined to the token's own event via server-side lookups. | anonymous (token/code only) |
...
hosted by an `is_league_host` club, OR any UCG-hosted event (FlipFest/Nationals — these need
no host club at all as of the 2026-07-22 feedback pass, `eventIsRefundEligible`). A
Dashboard-issued refund (bypassing this flow) still
does not automatically reflect back into `payments.status`/fulfillment — **F4's
`reconcile-payments` `scan`/`mark-refunded` ops (Finance page "Reconciliation" tab, live since 2026-07-18) close this loop as a manual admin action**, not an automatic one.
...
- **Fresh-backend boot order gotcha:** seed `seasons` BEFORE any sign-in. Since P3
  (2026-07-20) a season-less backend no longer crashes `Layout.tsx`/`Membership.tsx`/
  `Home.tsx` (all null-safe on no current-by-date season), but most flows still assume
  at least one season row exists, and the first sign-in's `link_or_create_person` row
  defeats `syncFromSupabase`'s empty-remote guard, wiping the local seed you'd push from.
